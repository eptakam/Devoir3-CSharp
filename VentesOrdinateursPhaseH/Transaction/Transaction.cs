/*
        ...
        But:            Couche métier 
                        ...
                        Adapter l'application à la culture courante 
...
namespace TransactionNS
{
    ...
    public class Transaction
    {
        ...
        public enum CodeErreurs
        {
            ...
            cePrixPositif,
            ceMarqueIntrouvable,
            ceFormatMarqueInvalide,
            ceErreurIndetermineeMarque,
            ceCaracteristiqueIntrouvable,
            ceFormatCaracteristiqueInvalide,
            ceErreurIndetermineeCaracteristique,
            cePrixIntrouvable,
            ceFormatPrixInvalide,
            ceErreurIndetermineePrix,
            ceErreurIndeterminee
        }

        public static string[] tMessagesErreursString = new string[26];
        ...
        public static void InitMessagesMessagesErreurs()
        {
            ...
            tMessagesErreursString[(int)ce.ceCodePostalIncorrect] = "Le format du code postal est incorrect";   //5
            tMessagesErreursString[(int)ce.ceTelephoneObligatoire] = "Le numero de telephone est obligatoire";  //6
            tMessagesErreursString[(int)ce.ceTelephoneInvalide] = "Le format du numéro de téléphone n'est pas valide";  //7
            tMessagesErreursString[(int)ce.ceDateLivraisonInvalide] = "La date de livraison est invalide."; // 8
            tMessagesErreursString[(int)ce.ceMarqueInvalide] = "La marque de l'ordinateur est invalide. Choisir une marque acceptable."; // 9
            tMessagesErreursString[(int)ce.ceCaracteristiqueInvalide] = "La caracterstique de l'ordinateur est invalide. Choisir une caracteristique acceptable."; // 10
            tMessagesErreursString[(int)ce.ceMarqueObligatoire] = "La marque de l'ordinateur est obligatoire."; // 11
            tMessagesErreursString[(int)ce.ceCaracteristiqueObligatoire] = "La caracteristique de l'ordinateur est obligatoire."; // 12
            tMessagesErreursString[(int)ce.cePrixOrdinateurInvalide] = "Le prix doit correspondre à la marque et à la caracteristique sélectionnée."; // 13
            tMessagesErreursString[(int)ce.cePrixObligatoire] = "Le prix est obligatoire."; // 14
            tMessagesErreursString[(int)ce.cePrixPositif] = "Le prix est un entier positif différent de zéro."; // 15
            tMessagesErreursString[(int)ce.ceMarqueIntrouvable] = "Le fichier des marques n'est pas disponible."; // 16
            tMessagesErreursString[(int)ce.ceMarqueInvalide] = "Le format de la marque n'est pas valide."; // 17
            tMessagesErreursString[(int)ce.ceErreurIndetermineeMarque] = "Erreur indéterminée dans la lecture des marques d'ordinateurs."; // 18
            tMessagesErreursString[(int)ce.ceCaracteristiqueIntrouvable] = "Le fichier des caracteristiques n'est pas disponible."; // 19
            tMessagesErreursString[(int)ce.ceCaracteristiqueInvalide] = "Le format de la caracteristique n'est pas valide."; // 20
            tMessagesErreursString[(int)ce.ceErreurIndetermineeCaracteristique] = "Erreur indéterminée dans la lecture des caracteristiques d'ordinateurs."; // 21
            tMessagesErreursString[(int)ce.cePrixIntrouvable] = "Le fichier des prix n'est pas disponible."; // 22
            tMessagesErreursString[(int)ce.ceFormatPrixInvalide] = "Le format du prix n'est pas valide."; // 23
            tMessagesErreursString[(int)ce.ceErreurIndetermineePrix] = "Erreur indéterminée dans la lecture des prix d'ordinateurs."; // 24
            tMessagesErreursString[(int)ce.ceErreurIndeterminee] = "Une erreur indeterminée. Communiquer avec la personne ressource."; // 25
        ...
        private System.Globalization.CultureInfo englishCultureInfo = new System.Globalization.CultureInfo("en-CA");
        ...
        {
            tMarqueStr = new string[40]; 
            StreamReader marqueStreamReader = null;

            try
            {
                string path = "..\\..\\Data\\Marques.data";   // Stocker le chemin d'accès 
                // Spécifier le marque d'Encoding désiré
                using (marqueStreamReader = new StreamReader(path, Encoding.UTF8))
                {
                    int nbMarquesInt;

                    // Lire le nombre d'enregistrement de Marques.data
                    string[] tFichierMarquesStr = File.ReadAllLines(path);
                    nbMarquesInt = tFichierMarquesStr.Length;

                    Array.Resize(ref tMarqueStr, nbMarquesInt); // Redimensionner le tableau

                    for (int indiceMarqueInt = 0; indiceMarqueInt < nbMarquesInt; indiceMarqueInt++)
                        tMarqueStr[indiceMarqueInt] = marqueStreamReader.ReadLine();
                }
            }
            catch (FileNotFoundException)
            {
                throw new FileNotFoundException(tMessagesErreursString[(int)ce.ceMarqueIntrouvable]);
            }
            catch (FormatException)
            {
                throw new FormatException(tMessagesErreursString[(int)ce.ceFormatMarqueInvalide]);
            }
            catch (Exception)
            {
                throw new Exception(tMessagesErreursString[(int)ce.ceErreurIndetermineeMarque]);
            }
        }

        public void InitCaracteristique()
        {
            tCaracteristiqueStr = new string[40];
            StreamReader caracteristiqueStreamReader = null;

            try
            {
                string path = "..\\..\\Data\\Caracteristiques.data";   // Stocker le chemin d'accès 

                // Spécifier le caracteristique d'Encoding désiré
                using (caracteristiqueStreamReader = new StreamReader(path, Encoding.UTF8))
                {
                    int nbCaracteristiquesInt;

                    // Lire le nombre d'enregistrement de Caracteristiques.data
                    string[] tFichierCaracteristiquesStr = File.ReadAllLines(path);
                    nbCaracteristiquesInt = tFichierCaracteristiquesStr.Length;
                    
                    Array.Resize(ref tCaracteristiqueStr, nbCaracteristiquesInt); // Redimensionner le tableau

                    for (int indiceCaracteristiqueInt = 0; indiceCaracteristiqueInt < nbCaracteristiquesInt; indiceCaracteristiqueInt++)
                        tCaracteristiqueStr[indiceCaracteristiqueInt] = caracteristiqueStreamReader.ReadLine();
                }
            }
            catch (FileNotFoundException)
            {
                throw new FileNotFoundException(tMessagesErreursString[(int)ce.ceCaracteristiqueIntrouvable]);
            }
            catch (FormatException)
            {
                throw new FormatException(tMessagesErreursString[(int)ce.ceFormatCaracteristiqueInvalide]);
            }
            catch (Exception)
            {
                throw new Exception(tMessagesErreursString[(int)ce.ceErreurIndetermineeCaracteristique]);
            }
        }

        public void InitPrix()
        {
            tPrixDecimal = new Decimal[40, 40];
            StreamReader prixStreamReader = null;            

            try
            {
                // Spécifier les chemins d'accès les fichiers
                string pathMarques = "..\\..\\Data\\Marques.data";
                string pathCaracteristiques = "..\\..\\Data\\Caracteristiques.data";
                string pathPrix = "..\\..\\Data\\Prix.data";

                // Obtenir les marques, les caractéristiques et les prix à partir des fichiers
                string[] tFichierMarquesStr = File.ReadAllLines(pathMarques);
                string[] tFichierCaracteristiquesStr = File.ReadAllLines(pathCaracteristiques);
                string[] tFichierPrixStr = File.ReadAllLines(pathPrix);

                // Spécifier le caracteristique d'Encoding désiré
                using (prixStreamReader = new StreamReader(pathPrix, Encoding.UTF8))
                {
                    int rangee = tMarqueStr.Length;
                    int colonne = tCaracteristiqueStr.Length;
                    ResizeArray(ref tPrixDecimal, rangee, colonne); // Redimensionner le tableau tPrixDecimal

                    for (int indiceMarque = 0; indiceMarque < tFichierMarquesStr.Length; indiceMarque++)
                    {
                        for (int indiceCaracteristique = 0; indiceCaracteristique < tFichierCaracteristiquesStr.Length; indiceCaracteristique++)
                        {
                            int indexPrix = indiceMarque * tFichierCaracteristiquesStr.Length + indiceCaracteristique; // Index dans le tableau des prix

                            // Vérifier si l'index est valide avant de convertir et d'ajouter le prix
                            if (indexPrix < tFichierPrixStr.Length)
                                tPrixDecimal[indiceMarque, indiceCaracteristique] = Decimal.Parse(tFichierPrixStr[indexPrix], englishCultureInfo);
                        }
                    }
                }
            }
            ...
            {
                throw new FileNotFoundException(tMessagesErreursString[(int)ce.cePrixIntrouvable]);
            }
            catch (FormatException)
            {
                throw new FormatException(tMessagesErreursString[(int)ce.ceFormatPrixInvalide]);
            }
            catch (Exception)
            {
                throw new Exception(tMessagesErreursString[(int)ce.ceErreurIndetermineePrix]);
            }
        }

        /// <summary>
        /// Permet de modifier la taille d'un tableau.
        /// </summary>
        /// <param name="array">Tableau a être modifié.</param>
        /// <param name="x">Nombre de rangées.</param>
        /// <param name="y">Nombre de colonnes.</param>
        /// <remarks>Array.Resize() fonctionne seulement avec un tableau à une dimension.</remarks>
